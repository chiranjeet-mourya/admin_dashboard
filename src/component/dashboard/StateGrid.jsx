      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4">
        {stats.map((state, index) => {
          return (
            <div
              key={index}
              className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-xl rounded-2xl p-4 border border-slate-200/50 dark:border-slate-700/50 hover:shadow-xl hover:shadow-slate-200/50 dark:hover:shadow-slate-900/20 transition-all duration-300 group"
            >
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="flex items-center space-x-2">
                    ...
                    <span className={`text-sm font-semibold ${state.trend === "up" ? "text-emerald-500" : "text-red-500"}`}>{state.change}</span>
                ...
                >
                    <div className={`w-6 h-6 flex items-center justify-center ${state.textColor}`}>
                        {state.icon}
                    </div>
                </div>
              </div>
